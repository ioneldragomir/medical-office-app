...
router
  ...
  .get(async (req, res) => {
    try {
      ...
      if (appointment) {   
        return res.status(200).json(appointment.appointmentFile);
      } else {
  ...
  .post(upload.single("file"), async (req, res) => {
    try {
      const appointmentFile = req.file;
      ...
      const appointment = await Appointments.findByPk(req.params.appointmentId);
      if (appointment) {   
        appointment.appointmentFile= appointmentFile.buffer;
  ...
  .delete(async (req, res) => {
    try {
      const appointment = await Appointments.findByPk(req.params.appointmentId);
      if (appointment) {   
        appointment.appointmentFile= null;
        const updatedAppointment = await appointment.save();
